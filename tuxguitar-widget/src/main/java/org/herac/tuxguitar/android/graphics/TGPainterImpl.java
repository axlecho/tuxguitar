...
public class TGPainterImpl extends TGResourceFactoryImpl implements TGPainter {
	...
	public void drawStringCenter(String text,float x,float y) {
        Rect r = new Rect();
        ...
        float rx = x - r.width() / 2f - r.left;
        float ry = y + r.height() / 2f - r.bottom;

        this.canvas.drawText(text, rx,ry, this.paint);
	}
